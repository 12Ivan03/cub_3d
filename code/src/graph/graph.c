...
int32_t	take_pixel(t_game **game, int wall, int x, int y)
{
	...
	p = &(*game)->graph->walls[wall]->pixels[4 * \
		(y * (*game)->graph->walls[wall]->width + x)];	
...
int	draw_col(t_game **game, t_line line, int ray_x, float curr_ang)
{
	...
	if (line.hit % 2 != 0)
	{
		colonm_num = ((line.a.x - floor((line.a.x + 1.00f) / GRID_SIZE) * \
		GRID_SIZE ) / GRID_SIZE);
		dist = distance(PLAYER.position, line.a);
	}
	else
	{
		colonm_num = ((line.b.y - floor((line.b.y + 1.00f) / GRID_SIZE) * \
		GRID_SIZE) / GRID_SIZE);
